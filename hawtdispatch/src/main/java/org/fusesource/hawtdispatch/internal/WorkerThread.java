...
final public class WorkerThread extends Thread {
    ...
    protected void onStart() {
        ...
        // isolation
        queue = new Work[INITIAL_QUEUE_CAPACITY];
    ...
    protected void onTermination(Throwable exception) {
        ...
        try {
            ...
            if (exception != null)
                WorkerPool.rethrowException(exception);
        }
    }
     ...
     */
    private static void setSlot(Work[] q, int i,
                                Work t) {
     ...
     */
    private static boolean casSlotNull(Work[] q, int i,
                                       Work t) {
     ...
     */
    final Work locallyDeqTask() {
        Work work = dispatchQueue.poll();
        if( work!=null ) {
            return work;
        }
        int b;
     ...
     * @param t the task. Caller must ensure non-null.
     */
    final boolean unpushTask(Work t) {
        Work[] q = queue;
        int mask = q.length - 1;
        int s = sp - 1;
    ...
    private void growQueue() {
        ...
        Work[] newQ = queue = new Work[newSize];
    ...
    private Work scan() {
        ...
        WorkerThread[] ws;       // refreshed on outer loop
     ...
     */
    final int drainTasksTo(Collection<? super Work> c) {
        int n = 0;
        Work t;
     ...
     */
    static boolean hasQueuedTasks(WorkerThread[] ws) {
        if (ws != null) {
            int len = ws.length;
     ...
     */
    final Work scanWhileJoining(Work joinMe) {
        Work t = pollTask();
    ...
    private static final long spOffset =
        objectFieldOffset("sp", WorkerThread.class);
    private static final long runStateOffset =
        objectFieldOffset("runState", WorkerThread.class);
    private static final long qBase;
    private static final int qShift;

    static {
        qBase = UNSAFE.arrayBaseOffset(Work[].class);
        int s = UNSAFE.arrayIndexScale(Work[].class);
        if ((s & (s-1)) != 0)
